...
### 3-1. `explore`만 고집하지 말고 `generalPurpose` 등 실행 가능한 타입을 검토하라

QA용 Task가 `explore`에만 고정되면 읽기 위주로 끝나기 쉽다. 하지만 효과적인 QA는:
- Grep으로 패턴 검색 (모든 `NextResponse.json()` 추출)
- 스크립트 실행으로 자동 대조 (API shape vs 훅 타입)
- 필요 시 수정까지 가능

**권장**: `generalPurpose` Task로 설정하되, 역할 `.md`에서 "검증 → 리포트 → 수정 요청" 프로토콜을 명시.
