...
> **Cursor 적용:** 아래 의사코드의 `TeamCreate`, `SendMessage`, `TaskCreate` 등은 **Claude Code 전용 API**이다. Cursor에서는 **메인 에이전트가 `Task`를 병렬·순차 호출**하고, 역할 간 정보 공유는 **`_workspace/` 파일 + 메인 통합**으로 구현한다. 대응 표는 `cursor-runtime.md` 참고.
...
| 팀원 | 에이전트 타입 | 역할 | 출력 |
|------|-------------|------|------|
| official-researcher | generalPurpose | 공식 문서/블로그 | research_official.md |
| media-researcher | generalPurpose | 미디어/투자 | research_media.md |
| community-researcher | generalPurpose | 커뮤니티/SNS | research_community.md |
| background-researcher | generalPurpose | 배경/경쟁/학술 | research_background.md |
| (리더 = 오케스트레이터) | — | 통합 보고서 | 종합보고서.md |

> 리서치 역할은 Task 호출 시 `generalPurpose` 등을 쓰되, 반드시 `.cursor/harness/agents/{name}.md`에 역할·조사 범위·`_workspace/` 출력 규칙을 명시한다. Cursor에서는 팀원 간 메시지 대신 파일로 상태를 공유한다.
...
### 에이전트 정의 파일
위치: `프로젝트/.cursor/harness/agents/{agent-name}.md`
